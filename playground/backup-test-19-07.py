...
def interfaceOutput():
    ...
    for ip in ipList:
        ...
        print '<strong>' + ip + '</strong>'
        print '''<table class="table is-bordered is-striped is-narrow is-hoverable is-fullwidth">
        <tr><th></th><td><strong>''' + ip + '''</strong></td><td></td><td></td><td></td><td></td><td></td></tr>
        <tr><th>#</th><td>Description</td><td>Type</td><td>MTU</td><td>Speed</td><td>AdminStatus</td><td>OperaStatus</td></tr>'''
        for temp in range(len(interface.result_desc)):
            ...
            <tr><td>{}</td><td>{}</td><td>{}</td><td>{}</td><td>{}</td><td>{}</td><td>{}</td></tr>'''.format(
                ...
                interface.result_Mtu[temp].value, interface.result_Speed[temp].value,
