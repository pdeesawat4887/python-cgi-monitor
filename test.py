...
def interface():
  ...
  <tr>
    ...
    for i in range(len(result_desc)):
        print '<tr><td>{}</td><td>{}</td><td>{}</td><td>{}</td><td>{}</td><td>{}</td>'.format(result_desc[i].value, typeChange(int(result_Type[i].value)), result_Mtu[i].value, round(
                float(result_Speed[i].value) / 1048576, 2), statusChange(int(result_Admin[i].value)), statusChange(
            int(result_Opera[i].value)))
