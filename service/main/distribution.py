...
class Server(maria_db.MySQLDatabase):
    ...
    def update_probe_alive(self, probe_id, ip_address):
        result = self.check_ping(ip_address)
        if result:
            re_result = 0
        else:
            re_result = 1
        update_sql = "UPDATE probe SET status='{}' WHERE probe_id='{}'".format(re_result, probe_id)
    ...
    def check_probe(self):
        ...
        for probe in self.all_probe:
            ...
            self.update_probe_alive(probe_id, ip_address)
